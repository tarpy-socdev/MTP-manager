...
show_resource_live() {
    ...
    trap 'tput cnorm 2>/dev/null; trap - INT TERM; return' INT TERM

    local proxy_port secret server_ip
    proxy_port=$(grep -oP '(?<=-H )\d+' "$SERVICE_FILE" 2>/dev/null || echo "N/A")
    secret=$(grep -oP '(?<=-S )\S+' "$SERVICE_FILE" 2>/dev/null || echo "N/A")
    ...
    while true; do
        # Проверка нажатия q
        read -t 0.05 -rsn1 key 2>/dev/null
        if [[ "$key" == "q" || "$key" == "Q" ]]; then
            break
        fi
        ...
        # PID и метрики
        local pid cpu mem rss_mb uptime_str connections
        ...
        if [ -n "$pid" ] && [ "$pid" != "0" ] && kill -0 "$pid" 2>/dev/null; then
            ...
            # Аптайм
            local start_time start_epoch now_epoch diff hh mm ss
            ...
            # Соединения
            connections=$(ss -tn state established 2>/dev/null | grep -c ":$proxy_port" 2>/dev/null || echo "0")
        ...
        printf " ║     %-38s ║\n" "$(date '+%H:%M:%S')  [q — выход в меню]"
        echo " ╚════════════════════════════════════════════╝"
        echo -e "${NC}"
        echo -e " Статус:       $(echo -e "$svc_status")"
        echo -e " Сервер:       ${CYAN}${server_ip}:${proxy_port}${NC}"
        echo -e " Аптайм:       ${CYAN}${uptime_str}${NC}"
        echo -e " Соединений:   ${CYAN}${connections}${NC}"
        echo ""
        echo -e " CPU: $(echo -e "$cpu_bar") ${CYAN}${cpu}%${NC}    "
        echo -e " RAM: $(echo -e "$mem_bar") ${CYAN}${mem}%${NC} (${rss_mb} MB)    "
        echo ""
        echo -e " ${BOLD}📝 Последние логи:${NC}"
        echo " ─────────────────────────────────────────────"
        journalctl -u mtproto-proxy -n 5 --no-pager 2>/dev/null | sed 's/^/ /' || echo " Логи недоступны"
        echo "                                              "
        echo -e " ${YELLOW}[q]${NC} — выход в меню                          "
