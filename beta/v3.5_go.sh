...
manager_show_status() {
    ...
    INTERNAL_PORT=$(grep -oP '(?<=-p )\d+' "$SERVICE_FILE" 2>/dev/null || echo "8888")
    RUN_USER=$(grep "^User=" "$SERVICE_FILE" 2>/dev/null | cut -d'=' -f2)
    ...
    if grep -q -- "-P " "$SERVICE_FILE" 2>/dev/null; then
        SPONSOR_TAG=$(grep -oP '(?<=-P )\S+' "$SERVICE_FILE" 2>/dev/null || echo "N/A")
    ...
    echo " ─────────────────────────────────────────────"
    ps aux 2>/dev/null | grep mtproto-proxy | grep -v grep | awk '{printf " PID: %s | CPU: %s%% | MEM: %s%%\n", $2, $3, $4}' || echo " Процесс не найден"
